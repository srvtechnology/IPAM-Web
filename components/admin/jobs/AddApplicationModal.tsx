...
export default function AddApplicationModal({ jobId, onClose }: { jobId: string; onClose: () => void }) {
  const { addApplication, loading, error } = useAdminJobs();
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const result = await addApplication(jobId, form);
    if (result) onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-headline-md text-on-surface">Register Candidate</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="col-span-2 font-body-compact text-on-surface-variant">
            Candidate name
            <input required value={form.candidateName} onChange={(e) => set("candidateName", e.target.value)} className={inputClass} />
          </label>
          <label className="font-body-compact text-on-surface-variant">
            Degree
            <input required value={form.degree} onChange={(e) => set("degree", e.target.value)} className={inputClass} />
          </label>
          <label className="font-body-compact text-on-surface-variant">
            Faculty
            <input required value={form.faculty} onChange={(e) => set("faculty", e.target.value)} className={inputClass} />
          </label>
          <label className="font-body-compact text-on-surface-variant">
            Graduation year
            <input required type="number" value={form.gradYear} onChange={(e) => set("gradYear", Number(e.target.value))} className={inputClass} />
          </label>
          <label className="font-body-compact text-on-surface-variant">
            Email
