...
export default function VirtualIdModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/60 p-3 backdrop-blur-xs sm:p-4"
      onClick={onClose}
    >
      <div
        className="relative my-6 w-full max-w-xl rounded-3xl border border-slate-200 bg-white text-slate-800 shadow-xl"
      ...
      >
        ...
        <div className="flex gap-1 border-b border-slate-200 bg-slate-100 p-2">
          <button
            onClick={() => setActiveTab("virtual")}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-bold sm:text-sm ${
              activeTab === "virtual" ? "border border-slate-200 bg-white font-black text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-bold sm:text-sm ${
              activeTab === "qr" ? "border border-slate-200 bg-white font-black text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-bold sm:text-sm ${
              activeTab === "order" ? "border border-slate-200 bg-white font-black text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"
            }`}
          >
        ...
        <div className="max-h-[80vh] overflow-y-auto p-5 sm:p-7">
          {activeTab === "virtual" && (
            <div className="space-y-5">
              <div className="text-center">
                <div className="mb-1 inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-emerald-800">
                  <ShieldCheck className="h-3 w-3 text-emerald-600" /> Verified Registrar Credential
                </div>
                <h3 className="text-xl font-black text-slate-900 sm:text-2xl">IPAM Alumni Virtual Card</h3>
                  ...
                  <div className="aspect-[1.586/1] w-full overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 p-4 text-white shadow-lg sm:p-5">
                    ...
                    <div className="flex items-center justify-between gap-3 py-1">
                      <div className="space-y-1">
                        ...
                        <div className="text-[8px] text-slate-400">Scan with any smartphone camera for live registrar verification.</div>
                      </div>
                      <div
                        onClick={(e) => { e.stopPropagation(); setActiveTab("qr"); }}
                        className="shrink-0 rounded-xl border border-white/40 bg-white p-1.5 shadow-xs"
                      >
              ...
              <div className="grid grid-cols-2 gap-2.5 pt-1">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white py-2.5 text-xs font-bold text-slate-700 shadow-xs hover:bg-slate-50"
                >
                  {copiedLink ? (<><Check className="h-4 w-4 text-emerald-600" /> Link Copied!</>) : (<><Copy className="h-4 w-4" /> Copy Card URL</>)}
                </button>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-2 text-xs">
                ...
                <button onClick={() => setActiveTab("order")} className="ml-auto flex items-center gap-1 font-semibold text-slate-500 hover:underline">
          ...
          {activeTab === "qr" && (
            <div className="space-y-6">
              <div className="text-center">
                <div className="mb-1 inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-emerald-800">
                  <QrCode className="h-3.5 w-3.5 text-emerald-600" /> Instant Camera Scanning
                </div>
                <h3 className="text-xl font-black text-slate-900 sm:text-2xl">Share Your QR & Digital Pass</h3>
              ...
              <div ref={qrRef} className="relative mx-auto max-w-xs space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-6 text-center shadow-xs">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-6 w-6 items-center justify-center rounded-md bg-emerald-600 text-[10px] font-black text-white">IPAM</div>
                ...
                <div className="inline-block rounded-2xl border border-slate-200 bg-white p-4 shadow-xs">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <button onClick={handleNativeShare} className="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-slate-700 shadow-xs hover:border-emerald-600 hover:bg-slate-50">
                    <Share2 className="h-4 w-4 text-emerald-600" /> Quick Share
                  </button>
                  <a
                    href={`https://api.whatsapp.com/send?text=${encodeURIComponent(shareText)}`}
                    target="_blank" rel="noopener noreferrer"
                    className="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-slate-700 shadow-xs hover:border-emerald-600 hover:bg-slate-50"
                  >
                  ...
                  <button onClick={handleDownloadQR} className="flex flex-col items-center gap-1 rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-slate-700 shadow-xs hover:border-emerald-600 hover:bg-slate-50">
              ...
              <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                {dispatchSent ? (
                  <div className="space-y-1 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-center">
                    <p className="text-xs font-bold text-slate-900">Digital Credential & QR Dispatched!</p>
                    ...
                    <button
                      ...
                      className="mx-auto block pt-1 text-[11px] font-bold text-emerald-700 hover:underline"
                    >
                  ...
                  <form onSubmit={handleDispatch} className="space-y-2">
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-900 focus:outline-emerald-600"
                      />
                      <input
          ...
          {activeTab === "order" && (
            <div className="space-y-4">
              <div className="mb-2 text-center">
                <h3 className="text-xl font-black text-slate-900">Order Physical Alumni ID Card</h3>
              </div>
