...
function JobCard({ job, initialSaved }: { job: JobListItem; initialSaved: boolean }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-xs hover:shadow-md transition-all duration-300 group hover:border-emerald-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      <div className="space-y-2 flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {job.postedByAlumni && (
            <span className="text-[11px] font-bold bg-teal-50 text-teal-800 px-2.5 py-0.5 rounded-full border border-teal-200 flex items-center gap-1">
              <Sparkles className="w-2.5 h-2.5 text-teal-600" /> Alumni Referrer: {job.postedByAlumni.name} (&apos;
      ...
      <div className="flex items-center gap-3 w-full md:w-auto shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100">
        <button
          onClick={toggle}
          disabled={loading}
        ...
        <Link
          href={`/jobs/${job.id}`}
          className="flex-1 md:flex-initial bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-extrabold text-sm hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 shadow-xs active:scale-98"
        >
      ...
      <section className="relative flex min-h-[580px] w-full items-center overflow-hidden border-b border-slate-800 bg-slate-950 text-white md:min-h-[640px]">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-6 px-4 py-16 sm:px-6 sm:py-20 md:py-24 lg:px-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-950/80 px-3.5 py-1.5 text-xs font-semibold text-emerald-300 shadow-xs backdrop-blur-md">
            ...
            <span>Alumni Career Network • Verified Corporate Opportunities</span>
          </div>

          <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div className="max-w-3xl space-y-3">
              <h1 className="text-3xl font-black leading-tight tracking-tight text-white sm:text-4xl md:text-5xl">
                IPAM Careers & Job Matching Portal
              </h1>
              <p className="text-base font-normal leading-relaxed text-slate-300 sm:text-lg">
                Exclusive career opportunities posted directly by alumni hiring managers, corporate leaders, and
                institutional partners. Fast-track your application with built-in alumni referral tags.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-slate-800/80 pt-4 sm:grid-cols-4">
            <div>
              ...
              <div className="text-xs font-medium text-slate-300">Active Openings</div>
            </div>
            <div>
              <div className="text-2xl font-black text-emerald-400 sm:text-3xl">{employerCount}+</div>
              <div className="text-xs font-medium text-slate-300">Partner Employers</div>
            </div>
            <div>
              <div className="text-2xl font-black text-emerald-400 sm:text-3xl">45%</div>
              <div className="text-xs font-medium text-slate-300">Alumni Referral Rate</div>
            </div>
            <div>
              <div className="text-2xl font-black text-emerald-400 sm:text-3xl">100%</div>
              <div className="text-xs font-medium text-slate-300">Verified Positions</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="space-y-4">
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-slate-200">
            ...
            <p className="text-lg font-bold text-slate-900">No job opportunities found</p>
