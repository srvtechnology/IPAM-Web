  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16 text-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <div className="bg-white rounded-3xl p-6 sm:p-8 lg:p-10 border border-slate-200/90 shadow-xs">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="space-y-3 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 text-xs font-bold bg-teal-50 text-teal-800 px-3 py-1 rounded-full border border-teal-200">
              ...
              <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-xs sm:text-sm text-slate-500 pt-3 border-t border-slate-100">
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col gap-3 w-full lg:w-auto shrink-0">
              <a
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3.5 rounded-2xl font-extrabold text-sm text-center shadow-xs transition-all flex items-center justify-center gap-2"
              >
              ...
              {session && (
                <button
                  ...
                  className={`px-5 py-3 rounded-2xl font-bold text-xs sm:text-sm transition-all flex items-center justify-center gap-2 border disabled:opacity-60 ${
                    saved ? "bg-emerald-50 border-emerald-300 text-emerald-800" : "bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100"
                  }`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 space-y-8">
            {job.postedByAlumni && (
              <div className="bg-white rounded-3xl p-6 sm:p-7 border border-emerald-200 shadow-xs space-y-4">
                <div className="flex items-start sm:items-center justify-between gap-4 flex-col sm:flex-row">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 text-[11px] font-bold uppercase tracking-wider mb-1 border border-emerald-200">
                        <Sparkles className="w-3 h-3 text-emerald-600" />
                        Hiring Sponsor &amp; Alumni Referrer
                      </div>
                      <h3 className="text-base sm:text-lg font-extrabold text-slate-900">{job.postedByAlumni.name}</h3>
                  ...
                  <span className="bg-emerald-600 text-white text-xs font-black px-3.5 py-1.5 rounded-full uppercase tracking-wider shadow-xs shrink-0">
                    Priority Route
                  </span>
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs space-y-4">
              ...
              <p className="text-sm sm:text-base text-slate-600 leading-relaxed whitespace-pre-line">{job.description}</p>
            </div>
                ...
                <ul className="space-y-3 text-sm sm:text-base text-slate-600">
                  {job.responsibilities.map((resp, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <span className="w-5 h-5 rounded-full bg-emerald-50 text-emerald-800 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold border border-emerald-200">
                        {index + 1}
                      </span>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <div className="flex flex-wrap gap-2">
                  {job.requirements.map((req, idx) => (
                    <span key={idx} className="text-xs font-semibold bg-slate-100 text-slate-700 px-3 py-1.5 rounded-xl border border-slate-200">
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {job.benefits.map((benefit, idx) => (
                    <div key={idx} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-start gap-3 text-xs sm:text-sm text-slate-700">
            ...
            {job.aboutCompany && (
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs space-y-3">
                ...
                <p className="text-sm sm:text-base text-slate-600 leading-relaxed">{job.aboutCompany}</p>
              </div>
            )}
          </div>

          <div className="space-y-6 lg:sticky lg:top-32">
            <div id="application-section" className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/90 shadow-xs space-y-5">
              ...
              {!session ? (
                ...
                <div className="p-6 rounded-2xl bg-emerald-50 text-emerald-950 text-center space-y-3 border border-emerald-200">
                  <div className="w-14 h-14 rounded-full bg-emerald-600 text-white mx-auto flex items-center justify-center shadow-xs">
                    <CheckCircle2 className="w-8 h-8" />
                  </div>
                  <div>
                    <h4 className="font-extrabold text-lg text-slate-900">Application Received!</h4>
                  ...
                  <div className="p-3 bg-white rounded-xl border border-emerald-200 text-left text-xs space-y-1">
                    <div className="flex justify-between text-slate-500">
                      ...
                      <code className="font-mono font-bold text-emerald-700">{currentApplication.applicationRef}</code>
                    </div>
                    <div className="flex justify-between text-slate-500">
                      <span>Status:</span>
                  ...
                  <p className="text-[11px] text-emerald-800 font-medium">You will receive confirmation and interview updates at {session.email}.</p>
                </div>
              ) : (
                <form onSubmit={handleApply} className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-700 block mb-1">LinkedIn / Portfolio URL</label>
                    <input
                      type="text"
                      value={linkedinUrl}
                      onChange={(e) => setLinkedinUrl(e.target.value)}
                      placeholder="linkedin.com/in/yourprofile"
                      className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-emerald-600 focus:bg-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-emerald-600 focus:bg-white resize-none"
                    />
                  </div>

                  <div className="flex items-start gap-2 pt-1">
                    <input type="checkbox" id="alumni-endorse-check" defaultChecked className="mt-0.5 rounded text-emerald-600 focus:ring-emerald-500 accent-emerald-600" />
                  ...
                  <button
                    type="submit"
                    disabled={applying}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3.5 rounded-2xl font-extrabold text-sm transition-all shadow-xs disabled:opacity-70 flex items-center justify-center gap-2"
                  >
                    <Send className="w-4 h-4" />
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-xs space-y-3.5 text-xs">
              ...
              <div className="flex justify-between items-center py-1 border-t border-slate-100">
                ...
                <span className="font-bold text-rose-600">{new Date(job.deadline).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {relatedJobs.length > 0 && (
          <div className="pt-8 border-t border-slate-200 space-y-6">
            <div className="flex justify-between items-end">
              <div>
                <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">Explore More Openings</span>
                <h3 className="text-2xl font-extrabold text-slate-900 mt-1">Similar Alumni Career Opportunities</h3>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedJobs.map((otherJob) => (
                <Link
                  ...
                  className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-xs hover:shadow-md hover:border-emerald-300 transition-all duration-300 flex flex-col justify-between group"
                >
                  <div>
                    ...
                    <h4 className="text-lg font-bold text-slate-900 group-hover:text-emerald-700 transition-colors line-clamp-1">{otherJob.title}</h4>
                    <p className="text-xs font-semibold text-emerald-700 mt-0.5">{otherJob.company}</p>
                    <p className="text-xs text-slate-500 line-clamp-2 mt-2 leading-relaxed">{otherJob.description}</p>
                  </div>
