...
import { Star, ChevronRight, ArrowUpRight, Sparkles } from "lucide-react";
...
export default function HomeBusinessesSection({ businesses }: { businesses: HomeBusinessItem[] }) {
  ...
  return (
    <section className="space-y-8 rounded-3xl border border-slate-200/90 bg-white p-6 shadow-xs sm:p-10">
      <div className="flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-4 md:flex-row md:items-end">
        <div>
      ...
      {featured && (
        <div className="group flex flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-slate-50 shadow-xs transition-all duration-300 hover:border-emerald-300 hover:shadow-md md:flex-row">
          <div className="relative h-64 min-h-[300px] overflow-hidden md:h-auto md:w-1/2">
            ...
            <div className="absolute left-4 top-4 flex flex-wrap gap-1.5">
              {featured.category && (
                <span className="rounded-full border border-slate-200 bg-white/95 px-3 py-1 text-xs font-bold text-slate-900 shadow-xs backdrop-blur-md">
                  {featured.category}
                </span>
              )}
              <span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-800 shadow-xs backdrop-blur-md">
                {featured.industry}
              </span>
            </div>
          </div>

          <div className="flex flex-col justify-center space-y-4 bg-slate-50 p-6 md:w-1/2 md:p-10">
            ...
            <h3 className="text-2xl font-extrabold text-slate-900 transition-colors group-hover:text-emerald-700 sm:text-3xl">
              {featured.name}
            </h3>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1 text-xs font-semibold text-slate-500">
              <span>Founded by: <strong className="text-slate-900">{featured.founders}</strong> ({featured.classYear})</span>
              <span>•</span>
              <span>HQ: <strong className="text-slate-900">{featured.location}</strong></span>
            </div>

            <div className="pt-2">
              <Link
                href={`/businesses/${featured.id}`}
                className="flex w-fit items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-extrabold text-white shadow-xs transition-all hover:bg-emerald-700"
              >
