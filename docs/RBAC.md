...
Every `AuditLogEntry` carries a `tamperHash`: a real HMAC-SHA256 (`lib/audit.ts`'s `tamperHash()`, keyed by the server-only `AUDIT_HMAC_SECRET` env var) computed at write time over the entry's substantive fields — `timestamp`, `actorEmail`, `action`, `target`, `status`, `severity`, `details`, and the JSON-serialized `beforeState`/`afterState`. Covering `details` and the before/after state (not just identity/routing fields) matters because those are exactly what an auditor reviews to see "what actually happened" — an earlier version of this hash covered only `timestamp|actorEmail|action|target`, which meant tampering `details` or the before/after state went undetected; this was caught during final smoke-testing and fixed before this build was accepted. This replaces the legacy admin prototype's fake bit-shift/`Math.random()` hash, which was purely decorative.

`GET /api/admin/audit-logs/[id]/verify` recomputes this hash server-side from the entry's stored fields and compares it to the stored `tamperHash`, returning `{ valid: boolean }`. The secret never reaches the client — verification only ever happens on the server. This mechanism was verified against real tampering during the final smoke test: an audit row's `details` field was modified directly in MySQL, and "Verify Integrity" correctly reported the entry as invalid; after reverting, it reported valid again. This is a genuinely working integrity check, not a cosmetic one.
